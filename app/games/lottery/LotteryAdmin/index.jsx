...
import { fetchLottery, jackpotByPlayer, jackpotQuote, selectLottery, unwatchLottery, watchLottery } from ".."
...
const LotteryAdmin = ({ address }) => {
  ...
  const jackpot = jackpotQuote(lottery)
  const pot = clampEth(prize) - jackpot
  const jackpots = jackpotByPlayer(lottery) || {}
  const jackpotRows = _.orderBy(_.map(jackpots, (amount, player) => ({ player, amount })), ["amount"], ["desc"])
  ...
  return (
    <div className={cn("lottery-admin", "flex min-h-0 flex-1 flex-col gap-3 overflow-hidden")}>
      ...
      {jackpotRows.length > 0 &&
        <div className={cn("lottery-admin-jackpots", "rounded-[0.75rem] border border-cs-border bg-cs-surface px-3 py-2")}>
          <span className={cn("lottery-admin-jackpots-label", "mb-1 block text-[0.75rem] text-cs-muted")}>Jackpot</span>
          {_.map(jackpotRows, (row) => {
            const { player, amount } = row || {}
            let label = `${player.slice(0, 6)}…${player.slice(-4)}`
            if (account && ethers.getAddress(player) === ethers.getAddress(account)) label = "you"
            let amountClass = "lottery-admin-jackpot-amount shrink-0 font-headings text-cs-muted"
            if (amount > 0) amountClass = "lottery-admin-jackpot-amount shrink-0 font-headings text-cs-accent"
            return (
              <div
                key={player}
                className={cn("lottery-admin-jackpot-row", "flex items-center justify-between gap-2 py-0.5 text-[0.75rem]")}
              >
                <span className={cn("lottery-admin-jackpot-player", "truncate font-mono text-cs-body")}>{label}</span>
                <span className={cn(amountClass)}>+{ethLabel(amount, symbol)}</span>
              </div>
            )
          })}
        </div>
